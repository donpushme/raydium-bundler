...
import { logToFile, outputBalance, readBundlerWallets, sleep, solGatherWaiting } from "../src/utils";
...
export const each_sol_gather = async () => {
    screen_clear()
    logToFile(`Gathering Wsol from one of bundler wallets...`);
    ...
    rl.question("\t[Bundler] - Bundler wallet to gather wsol (if you want to go back, press c and press enter): ", async (answer: string) => {
        ...
        logToFile(`Balance of the wallet${walletIndex} SOL : ${solBalance / LAMPORTS_PER_SOL}sol, WSOL : ${wsolBalance}wsol`)
