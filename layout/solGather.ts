...
import { logToFile, outputBalance, readBundlerWallets, readJson, saveBundlerWalletsToFile, sleep, solGatherWaiting } from "../src/utils";
        ...
        for (let i = 0; i < batchNum; i++) {
            ...
            while (true) {
                try {
                    ...
                    if (txSig) {
                        successNum++
                        logToFile(`SOL gathered from bundler ${batchLength * i + 1}, ${batchLength * i + 2}, ${batchLength * i + 3}: ${tokenBuyTx}`)
                    }
                    break
                } catch (error) {
                    index++
