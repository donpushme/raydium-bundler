...
import { logToFile, mainMenuWaiting, readBundlerWallets, readHolderWallets, readJson, sleep, tokenHoldersWaiting } from "../src/utils";
        ...
        rl.question("\t[Bundlers] - Bundler wallets to gather tokens (if you want to go back, press c and press enter): ", async (answer: string) => {
            ...
            selectedBundlerWallets.map(async (bundler: string, i) => {
                ...
                for (let k = 0; k < batchNum; k++) {
                    ...
                    if (sig) {
                        logToFile(`Success gather to bundler ${numbers[i]}: https://solscan.io/tx/${sig}${cluster == "devnet" ? "?cluster=devnet" : ""}`)
                    }
