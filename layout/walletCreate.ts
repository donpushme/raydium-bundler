...
import { logToFile, mainMenuWaiting, outputBalance, prepareBundleWaiting, readBundlerWallets, readSwapAmounts, saveBundlerWalletsToFile, saveSwapSolAmountToFile, sleep } from "../src/utils";
...
export const wallet_create = async () => {
  screen_clear()
  logToFile(`Creating ${walletNum} Wallets for bundle buy`);
  ...
  if(bundlerProviderBal < totalSolRequired) {
    logToFile(`Total required Sol is ${totalSolRequired}sol, but bundler provider wallet has ${bundlerProviderBal}, please deposit more and retry.`)
  ...
  try {
    // logToFile(`Each bundler will be created every 20 seconds.`)
    for (let i = 0; i < bundleWalletNum; i++) {
      const newWallet = Keypair.generate()
      wallets.push(bs58.encode(newWallet.secretKey))
      logToFile(`Bundler ${i + 1} => ${newWallet.publicKey.toBase58()}`)
    }
