...
import { logToFile, outputBalance, readBundlerWallets, readJson, readLUTAddressFromFile, saveDataToFile, sellBuyWaiting, sleep } from "../src/utils"
    ...
    rl.question("\t[Solana] - Buy Sol Amount in each bundler wallet (if you want to go back, press c and press enter): ", async (answer: string) => {
        ...
        if (!params.marketId) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                logToFile(`Processing transaction ${i + 1}`)
                ...
                if (cluster == "devnet") {
                    ...
                    logToFile(`Token bought: ${tokenBuyTx}`)
                    logToFile("*******************************************************************************************")
                }
            }
            ...
            if (cluster == "mainnet") {
                logToFile("------------- Bundle & Send ---------")
                logToFile("Please wait for 30 seconds for bundle to be completely executed by all nearests available leaders!");
                let result;
                while (1) {
                    result = await jitoWithAxios(versionedTxs, LP_wallet_keypair)
                    if (result.confirmed) {
                        logToFile(`Bundle signature: ${result.jitoTxsignature}`)
                        break;
                    }
                }
                logToFile("------------- Bundle Successfully done ----------");
            }
            ...
            logToFile(`Total Token Balance after rebuy: ${totalTokenBalance}`)
...
const distributeSol = async (buySolAmount: number) => {
    ...
    try {
        for (let i = 0; i < batchNum; i++) {
            ...
            while (true) {
                try {
                    ...
                    if (txSig) logToFile(`SOL distributed: ${txResult}`)
                    else logToFile("Fail to distribute sol")
                    break
                } catch (error) {
                    index++
                    // console.log(error)
        ...
        logToFile(`Failed to transfer SOL`)
    }
    ...
    try {
        ...
        walletKPs.map(async(wallet: Keypair, i: any) => {
            ...
            while (true) {
                try {
                    ...
                    const txResult = txSig ? `https://solscan.io/tx/${txSig}${cluster == "devnet" ? "?cluster=devnet" : ""}` : ''
                    if (txSig) logToFile(`WSOL transaction success in bundler ${i} ${txResult}`)
                    else logToFile("Fail to distribute sol")
                    break
                } catch (error) {
                    index++
        ...
        logToFile(`Failed to transfer to wsol`)
    }
}