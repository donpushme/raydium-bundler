...
import { logToFile, readBundlerWallets, readHolderWallets, readJson, sleep, tokenHoldersWaiting } from "../src/utils";
    ...
    try {
        ...
        bundlerWallets.map(async (bundler: string, i) => {
            ...
            for (let k = 0; k < batchNum; k++) {
                ...
                if (sig) logToFile(`Success gather to bundler ${i + 1}: https://solscan.io/tx/${sig}${cluster == "devnet" ? "?cluster=devnet" : ""}`)
            }
        })
