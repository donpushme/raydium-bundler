...
import { logToFile, mainMenuWaiting, outputBalance, prepareBundleWaiting, readBundlerWallets, readJson, readSwapAmounts, sleep } from "../src/utils";
    ...
    try {
        ...
        wallets.map((async (wallet, i) => {
            ...
            if (confirmation.value.err) {
                ...
                successTxNum++
                logToFile(`Wallet${i + 1}'s ata preparation tx: https://solscan.io/tx/${sig}${cluster == "devnet" ? "?cluster=devnet" : ""}`)
                if (successTxNum === wallets.length) {
                    logToFile("Ata creation finished")
                    return
                }
            }
        }))
        logToFile("Waiting for ata creation result")
        await sleep(35000)
        logToFile(`Successful ata creation for ${successTxNum} wallets`)
        // if (successTxNum === wallets.length) {
            logToFile("Ata creation finished")
            // return
        // } else {
            // logToFile(`${wallets.length - successTxNum} tx failed, try again`)
        // }
    } catch (error) {
...
export const create_atas = async () => {
    ...
    try {
        ...
        logToFile("Creating associated token accounts.")
