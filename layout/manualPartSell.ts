...
import { logToFile, outputBalance, readBundlerWallets, readJson, readLUTAddressFromFile, sellBuyWaiting, sleep } from "../src/utils"
import {
    connection,
    cluster,
} from "../config";
    ...
    rl.question("\t[Percent of total supply] - Sell Amount (if you want to go back, press c and press enter) : ", async (answer: string) => {
        ...
        if (!params.marketId) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (!lookupTable) {
                    logToFile("Error in fetching the data of the lookuptable.")
                }
            ...
            logToFile(`Total Token Balance after partial sell: ${totalTokenBalance}`)
