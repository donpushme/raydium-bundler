...
import { logToFile, outputBalance, prepareBundleWaiting, readBundlerWallets, readJson, saveLUTAddressToFile, sleep } from "../src/utils";
...
const createAndSendV0Tx = async (txInstructions: TransactionInstruction[]) => {
    try {
        ...
        // logToFile(`   ✅ - Fetched latest blockhash. Last valid height: ${latestBlockhash.lastValidBlockHeight}`);
        ...
        // logToFile(`   ✅ - Compiled transaction message`);
        const transaction = new VersionedTransaction(messageV0);

        // Step 3 - Sign your transaction with the required `Signers`
        transaction.sign([SIGNER_WALLET]);
        // logToFile(`   ✅ - Transaction Signed by the wallet ${(SIGNER_WALLET.publicKey).toBase58()}`);
        console.log(await connection.simulateTransaction(transaction))
        // Step 4 - Send our v0 transaction to the cluster
        const txid = await connection.sendTransaction(transaction, { maxRetries: 5 });
        // logToFile("   ✅ - Transaction sent to network");

        // Step 5 - Confirm Transaction 
        const confirmation = await confirmTransaction(connection, txid);
        // if (confirmation.value.err) { throw new Error("   ❌ - Transaction not confirmed.") }
        logToFile(`🎉 Transaction successfully confirmed!: https://explorer.solana.com/tx/${txid}${cluster == "devnet" ? "?cluster=devnet" : ""}`);
    } catch (error) {
        logToFile("There is an error in creating LUT. Press enter and retry this step again.")
...
async function createLUT() {
    try {
        ...
        logToFile(`Lookup Table Address: ${lookupTableAddress.toBase58()}`);
        ...
        createAndSendV0Tx([
            ...
            lookupTableInst]);
        logToFile("Lookup Table Address created successfully!")
        logToFile("Please wait for about 15 seconds...")
        await sleep(20000)
        return lookupTableAddress
    } catch (err) {
        logToFile("Error in creating Lookuptable. Please retry this.")
        prepareBundleWaiting()
    }
...
async function addAddressesToTable(LOOKUP_TABLE_ADDRESS: PublicKey, mint: PublicKey) {
    ...
    try {// Step 1 - Adding bundler wallets
        ...
        logToFile("Successfully added wallet addresses.")
        await sleep(20000)

        // Step 2 - Adding wallets' token ata
        logToFile(`Adding atas for the token ${mint.toBase58()}`)
        ...
        logToFile("Successfully added token ata addresses.")
        await sleep(5000)
        ...
        logToFile("Successfully added wsol ata addresses.")
        await sleep(10000)

        logToFile("Lookup Table Address extended successfully!")
        logToFile(`Lookup Table Entries: https://explorer.solana.com/address/${LOOKUP_TABLE_ADDRESS.toString()}/entries${cluster == "devnet" ? "?cluster=devnet" : ""}`)
    }
    catch (err) {
        logToFile("There is an error in adding addresses in LUT. But it is ok, so you can continue to the next step by pressing Enter.")
        prepareBundleWaiting()
        return;
    }
}
...
export const create_extend_lut = async () => {
    ...
    try {
        ...
        logToFile("Extending Address LookUpTable for our bundler.")
        ...
        // logToFile("Error occurred in creating lookuptable. Please retry this again.")
        prepareBundleWaiting()
    }
